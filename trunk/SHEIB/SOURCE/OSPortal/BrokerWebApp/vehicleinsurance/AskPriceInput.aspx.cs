...
namespace BrokerWebApp.vehicleinsurance
{
    public partial class AskPriceInput : BasePage
    {
            ...
            ASPxGridViewTableRowEventArgs e)
        {
            if (e.RowType == GridViewRowType.Data)
            {
                ...
                if (thectr != null)
                {
                    ...
                    thelnk.ID = "fileurl" + Convert.ToString(e.GetValue("CarPolicyDocID"));
                    thelnk.NavigateUrl = "#";
        ...
        protected void gridDocList_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
        {
            ...
            object theValues = this.gridDocList.GetRowValuesByKeyValue(theID, new String[] { "CarPolicyDocID", "DocName", "DocURL" });
            object[] theValueList = theValues as object[];
            String carPolicyDocID, docName, docURL;
            if (theValueList[0] == null)
                carPolicyDocID = "";
            else
                carPolicyDocID = theValueList[0].ToString();
            ...
            {
                string policyFolderPath;
                policyFolderPath = System.IO.Path.Combine(MapPath(UploadDirectory), carPolicyDocID);
            ...
            {
                BusinessObjects.Policy.BO_CarPolicyDoc.Delete(theID);
            }
